...
func (g Group) getMemoryDir() string {
	return filepath.Join(g.cgroupDir, "memory", "harbor", g.name)
}

// /sys/fs/cgroup/memory/harbor/abc/pids.max
...
// /sys/fs/cgroup/pids/harbor/abc
func (g Group) getPidsDir() string {
	return filepath.Join(g.cgroupDir, "pids", "harbor", g.name)
}

// /sys/fs/cgroup/pids/harbor/abc/pids.max
func (g Group) getPidMaxFile() string {
	return filepath.Join(g.getPidsDir(), "pids.max")
}

// /sys/fs/cgroup/pids/harbor/abc/cgroup.procs
func (g Group) getProcsFile() string {
	return filepath.Join(g.getPidsDir(), "cgroup.procs")
}

// /sys/fs/cgroup/pids/harbor/abc/notify_on_release
